...
func (c *conn) serve(ctx context.Context) {
		...
		*c.tlsState = tlsConn.ConnectionState()
		if proto := c.tlsState.NegotiatedProtocol; validNextProto(proto) {
			if fn := c.server.TLSNextProto[proto]; fn != nil {
				...
				c.setState(c.rwc, http.StateActive, false /*skipHooks ~= false*/)
	...
	for {
		...
		if c.r.remain != c.server.initialReadLimitSize() {
			c.setState(c.rwc, http.StateActive, true /*runHooks ~= true*/)
		}
		if err != nil {
		...
		c.setState(c.rwc, http.StateIdle, true /*runHooks ~= true*/)
		c.curReq.Store(nil)
...
func (h initALPNRequest) OutOfBand(_ *http.Request) {}
...
func (c *conn) hijackLocked() (rwc net.Conn, buf *bufio.ReadWriter, err error) {
	...
	c.setState(rwc, http.StateHijacked, true /*runHooks ~= true */)
	return
}
...
func (f HandlerFunc) OutOfBand(_ *http.Request) {}
